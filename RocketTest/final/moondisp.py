...
def orbDisplay(par, totalt):
    ...
    while ((X_r - moon.X)**2+(Y_r - moon.Y)**2)**0.5<par[2]:#abs(t)<totalt:#checkPhi<10*np.pi:
        print(t)
        Phase=par[1]
        RtoMoon = par[2]
        ...
        if curPhase < 0.005 and started == 0:
            started = 1
            # print('staaart')
        if ((X_r - moon.X)**2+(Y_r - moon.Y)**2)**0.5 <= RtoMoon and started == 1:
            F_r = -13000
    ...
    for planet in PS.Planets:
        planet.TraceX = np.array([planet.X])
        planet.TraceY = np.array([planet.Y])
    PS.rocket.TraceX = np.array([X_r])
    PS.rocket.TraceY = np.array([Y_r])
    PS.EquationsOfMovement = 'Null'
    PS.EquationsOfRocket = 'Null'
    dict = {'PS': PS}
...
tempParams = [-0.001, 0.75+np.pi/4, 7.5, 40000]#5
# tempParams = [-0.001, -2.7+np.pi/2, 8, 400000]#5
# tempParams = [-0.001, 1.8+np.pi/2, 8.5, 800000]#5
# tempParams = [-0.001, 0.+np.pi/2, 9, 1600000]#5
# tempParams = [-0.001, -1.4+np.pi/2, 9.5, 3200000]#5
