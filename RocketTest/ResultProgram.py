...
class ExampleApp(QMainWindow, LoadSystem.Ui_MainWindow):
    ...
    def LoadSystemButtonClicked(self):
        global dt, totalT, cadrSkip, eee


        dt = float(self.dtInput.toPlainText())
        totalT = float(self.tLimInput.toPlainText())
        cadrSkip = float(self.cadrSkipField.toPlainText())
        print('dt:', dt)
        print('totalT:', totalT)
        print('cadrSkip:', cadrSkip)
        ...
        ax = self.SpaceWidget.canvas.axes
        ...
        tempParams =[0.001, 50.31927001953132, -0.5710937500000008, 8.252675781250007, 2.1456161087989405]
        res = Calculate(tempParams)
        Sys.DrawSystem(ax)

        anim = FuncAnimation(fig, partial(Kadr, Sys, Ea, Mo, Ro, res, ax), frames=np.linspace(0, 999999, 10000), blit=True)
