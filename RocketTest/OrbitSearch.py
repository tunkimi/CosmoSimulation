...
def orbErr(par):
    ...
    while t<100:#checkPhi<10*np.pi:
        ...
        if firsthalfway:
            ...
            if moonstarted == 0 and ((Y_r - moon.Y) ** 2 + (X_r - moon.X) ** 2) ** 0.5 <= Rtomoon:
                ...
                dPhi = math.atan2(nY, nX)
                checkPhi += dPhi
                ErrSum += (0.3156254615 - ((Y_r - moon.Y) ** 2 + (X_r - moon.X) ** 2) ** 0.5)**2
                ErrN += 1
                prevPhi = math.atan2(Y_r - moon.Y, X_r - moon.X)
                moonended = 1
                fullRolls += dPhi
                # print('rolling')
            if moonended == 1 and ((Y_r - moon.Y) ** 2 + (X_r - moon.X) ** 2)**0.5 > 1:
                print('пролёт около луны')
                return 10000
                if ErrN == 0:
                    return 10000
                return 10000 * ErrSum/fullRolls
            ...
            if ((Y_r - moon.Y) ** 2 + (X_r - moon.X) ** 2)**0.5 < moon.Rc:
                print('столкновение')
                return 10000
                if ErrN == 0:
                    return 10000
    ...
    # return Err
    # print('t:', t)
    # print('check', checkPhi)
    if ErrN == 0:
        return 10000
    fullRolls /= 2*np.pi
    # print('err', ErrSum/ErrN)
    print('err', ErrSum/fullRolls)
    # return 10000 * ErrSum/ErrN
    return 10000 * ErrSum/fullRolls
    return ErrSum/ErrN

# print('result:', orbErr([0.001, 50.31632812500004, -0.56375, 7.960175781249993, 2.114366108798941]))
# print('result:', orbErr([0.001, 50.31632812500004, -2.56375, 7.960175781249993, 2.114366108798941]))


tempPars = [0.001, 50.31632812500004, -0.56375, 7.960175781249993, 2.114366108798941]
tempPars = [0.001, 50.31940429687504, -0.56875, 8.732675781250025, 2.15928798379894]
