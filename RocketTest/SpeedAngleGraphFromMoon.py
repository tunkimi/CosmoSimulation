...
for i in np.linspace(0,2*np.pi, 50):
    ...
    if inside==0:
        ...
        SpeedGraphY = np.append(SpeedGraphY, 0)
        AlphaGraphY = np.append(AlphaGraphY, 0)
        BetaGraphY  = np.append(BetaGraphY, 0)
    else:
        SpeedGraphX = np.append(SpeedGraphX, i)
    ...
    # print(i, ((PS.rocket.Vx - earth.Vx)**2+(PS.rocket.Vy - earth.Vy)**2)**0.5,
    #       VRM, ERM)

    # print(SpeedGraphY)
print(time.localtime(),  startTime)
fileway = 'C:/Users/iTunkimi/Desktop/GitReps/CosmoSimulation/RocketTest/fromMoon_0.01_0-2pi-50new.csv'
