    ...
    while t<100:#checkPhi<10*np.pi:
        ...
        if firsthalfway:
            ...
            if moonstarted == 0 and ((Y_r - moon.Y) ** 2 + (X_r - moon.X) ** 2) ** 0.5 <= Rtomoon:
                ...
                dPhi = math.atan2(nY, nX)
                checkPhi += dPhi
                ErrSum += neededOrb - ((Y_r - moon.Y) ** 2 + (X_r - moon.X) ** 2) ** 0.5
                ErrN += 1
                prevPhi = math.atan2(Y_r - moon.Y, X_r - moon.X)
                moonended = 1
                fullRolls += dPhi
...
calcOrb = (rasstkm*1000 + 1737100)/r0 #400*2**4км
print(calcOrb)

tempPars = [0.001, 50.31940429687504, -0.569375, 8.61267578125002, 2.14678798379894]
tempPars = [0.001, 50.32998016357378, -0.5701025390625051, 4.3686132812500755, 2.3835164994239535] #- идем к 400*2**4км
tempPars = [0.001, 55.43743896484375, -1.376953125, 28.333333333333336, 37.59397757685488] #37.59397757685488
